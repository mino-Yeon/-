...
### 해설
\`Supplier<String>\`도 마찬가지로 매개변수가 없습니다. 반환 타입만 \`String\`으로 바뀌었을 뿐, 람다 형태는 1번 문제와 동일하게 \`() -> 값\` 구조입니다.

---

id: ch1-03
title: Consumer로 출력하기
difficulty: easy
---
...
### 해설
\`Function<T, R>\`의 T와 R이 다른 타입이어도 괜찮습니다. 여기서는 \`String\`을 받아 \`Integer\`로 변환합니다. 입력 타입과 출력 타입이 다르다는 점에서 5번 문제와 대비해서 이해하면 좋습니다.

---

id: ch1-07
title: Predicate로 조건 판별하기
difficulty: easy
---
...
### 해설
\`Predicate\`는 단순 비교뿐 아니라 이번 문제처럼 메서드 호출 결과를 조건식에 활용할 수도 있습니다. \`s.length() >= 5\`처럼 boolean으로 평가되는 식이면 어떤 형태든 람다 본문에 올 수 있습니다.
`,S=`---
id: ch2-01
title: filter로 짝수만 걸러내기
difficulty: easy
---

### 요구사항
정수 리스트 \`numbers\`에서 짝수만 걸러 리스트로 만들어라.
...
### 해설
\`filter\`는 \`Predicate<T>\`를 받아 조건을 만족하는 요소만 남깁니다. 스트림의 각 요소가 하나씩 람다의 매개변수로 전달되고, \`boolean\`으로 평가되는 식이 본문에 와야 합니다.

---

id: ch2-02
title: map으로 대문자 변환하기
difficulty: easy
---

### 요구사항
문자열 리스트 \`names\`의 모든 이름을 대문자로 변환한 리스트를 만들어라.
...
### 실행용 데이터
\`\`\`java
List<String> names = Arrays.asList("jimin", "hana", "suho");
\`\`\`

### 정답
- BLANK1: \`s -> s.toUpperCase()\`

### 해설
\`map\`은 \`Function<T, R>\`을 받아 각 요소를 변환한 새로운 값으로 바꿉니다. 이번 문제는 \`String\`을 받아 \`String\`(대문자)으로 변환하므로 입력과 출력 타입이 같은 경우입니다.
...
### 요구사항
정수 리스트 \`numbers\`의 각 숫자를 제곱한 값으로 이루어진 리스트를 만들어라.
...
### 해설
\`map\`의 람다 본문은 변환 결과를 반환하는 식이면 됩니다. 여기서는 \`n\`을 받아 \`n * n\`이라는 계산 결과를 반환합니다.

---

id: ch2-04
title: filter로 조건에 맞는 문자열 남기기
difficulty: easy
---

### 요구사항
문자열 리스트 \`words\`에서 길이가 3보다 긴 단어만 남긴 리스트를 만들어라.
...
### 실행용 데이터
\`\`\`java
List<String> words = Arrays.asList("cat", "dog", "elephant", "ox", "tiger");
\`\`\`
...
### 해설
\`filter\`의 조건식 안에서 메서드 호출 결과(\`w.length()\`)를 활용할 수 있습니다. 조건이 boolean으로 평가되기만 하면 됩니다.

---

id: ch2-05
title: filter와 map 조합 (짝수를 골라 제곱하기)
difficulty: medium
---

### 요구사항
정수 리스트 \`numbers\`에서 짝수만 골라낸 뒤, 그 값들을 제곱한 리스트를 만들어라.
...
### 해설
\`filter\`로 조건에 맞는 요소만 먼저 골라낸 뒤, 그 결과를 \`map\`으로 변환합니다. 스트림 파이프라인은 위에서 아래로 순서대로 처리되므로, filter가 먼저 오고 map이 그다음에 오는 순서가 중요합니다.

---

id: ch2-06
title: filter와 map 조합 (긴 이름만 골라 길이 구하기)
difficulty: medium
---

### 요구사항
문자열 리스트 \`names\`에서 길이가 4 이상인 이름만 골라낸 뒤, 그 이름들의 길이로 이루어진 리스트를 만들어라.
...
### 실행용 데이터
\`\`\`java
List<String> names = Arrays.asList("jimin", "ha", "yerin", "su");
\`\`\`
...
### 해설
filter 조건과 map 변환에서 똑같이 \`length()\`를 사용하지만 역할이 다릅니다. filter에서는 boolean 판별에 쓰이고, map에서는 실제 반환할 값(Integer)을 만드는 데 쓰입니다.
`,C=`# 사전 준비

이 챕터의 문제를 풀기 전에, 아래 클래스들을 미리 만들어 두세요. (이 클래스들은 이후 챕터에서도 계속 재사용됩니다. 필드가 이번 챕터에서 다 쓰이지 않더라도 미리 전체를 만들어 두세요.)
...
public class Member {
    ...
    public Member(String id, String nickname, int points) {
        this.id = id;
...
id: ch3-01
title: 10 이상인 숫자만 걸러내기
difficulty: medium
---

### 요구사항
정수 리스트 \`numbers\`에서 10 이상인 숫자만 골라 리스트로 만들어라.
...
### 실행용 데이터
\`\`\`java
List<Integer> numbers = Arrays.asList(3, 7, 10, 15, 22, 8);
\`\`\`

### 정답
- BLANK1: \`.filter(n -> n >= 10)\`

### 해설
"~만 골라라"는 요구사항은 조건 판별이 필요하므로 \`filter\`가 필요합니다. 값을 바꾸는 게 아니라 남길지 말지만 결정하므로 \`map\`이 아니라 \`filter\`를 써야 합니다.

---

id: ch3-02
title: 이름을 모두 대문자로 바꾸기
difficulty: medium
---

### 요구사항
문자열 리스트 \`names\`의 모든 이름을 대문자로 바꾼 리스트를 만들어라.
...
### 실행용 데이터
\`\`\`java
List<String> names = Arrays.asList("chulsoo", "younghee", "minjun");
\`\`\`

### 정답
- BLANK1: \`.map(s -> s.toUpperCase())\`

### 해설
"~로 바꿔라"는 요구사항은 값 자체를 변환하는 것이므로 \`map\`이 필요합니다. 1번 문제와 대비해서, "걸러내기"와 "바꾸기"라는 표현의 차이가 각각 filter와 map으로 이어진다는 것을 기억해두면 좋습니다.

---

id: ch3-03
title: 가격 조건에 맞는 상품 이름 뽑기
difficulty: medium
---

### 요구사항
상품 리스트 \`products\`(각 상품은 \`name\`, \`price\`, \`stock\` 필드를 가짐)에서 가격이 10000원 이상인 상품의 이름만 뽑아 리스트로 만들어라.
...
List<Product> products = Arrays.asList(
    new Product("keyboard", 45000, 12),
    new Product("mouse", 8000, 30),
    new Product("monitor", 210000, 5),
    new Product("mousepad", 5000, 20)
);
\`\`\`

### 정답
- BLANK1: \`.filter(p -> p.getPrice() >= 10000)\`
- BLANK2: \`.map(p -> p.getName())\`

### 해설
요구사항을 나누어 읽으면 "가격이 10000원 이상인 상품"(조건 판별 → filter)과 "그 상품의 이름"(값 변환 → map) 두 동작이 순서대로 필요합니다. 조건으로 먼저 걸러낸 뒤 원하는 필드만 뽑아내는 순서가 자연스럽습니다.

---

id: ch3-04
title: 긴 단어만 남기기 (길이 계산 후 필터링)
difficulty: medium
---

### 요구사항
문자열 리스트 \`words\`에서 각 단어의 길이를 구한 뒤, 5보다 긴 것만 남긴 리스트를 만들어라.
...
### 실행용 데이터
\`\`\`java
List<String> words = Arrays.asList("cat", "elephant", "dog", "butterfly", "ox");
\`\`\`

### 정답
- BLANK1: \`.map(w -> w.length())\`
- BLANK2: \`.filter(len -> len > 5)\`

### 해설
요구사항 문장 순서 그대로 "길이를 구한 뒤(변환 → map) 5보다 긴 것만 남긴다(조건 판별 → filter)"입니다. 3번 문제와 달리 map이 먼저 오는 이유는, 걸러낼 기준(길이)이 원본 데이터에는 없고 변환을 거쳐야 생기기 때문입니다. filter/map 순서는 고정된 것이 아니라 요구사항의 논리적 순서를 따라야 합니다.

---

id: ch3-05
title: 고득점 학생 이름만 출력하기
difficulty: medium
---

### 요구사항
학생 리스트 \`students\`(각 학생은 \`name\`, \`score\`, \`grade\` 필드를 가짐)에서 점수가 90점 이상인 학생의 이름만 콘솔에 출력하라. (리스트로 만들지 않고 바로 출력한다)
...
List<Student> students = Arrays.asList(
    new Student("jimin", 95, 2),
    new Student("hana", 82, 3),
    new Student("suho", 91, 1)
);
\`\`\`

### 정답
- BLANK1: \`.filter(s -> s.getScore() >= 90)\`

### 해설
지금까지는 \`collect\`로 결과를 리스트로 모았지만, 이번에는 "출력하라"는 요구사항이므로 값을 모을 필요 없이 각 요소에 대해 동작만 실행하면 됩니다. 이럴 때는 \`Consumer\`를 받는 \`forEach\`가 최종연산으로 적합합니다. \`forEach\`는 \`collect\`와 달리 반환값이 없습니다. 이번 문제에서는 \`forEach\` 부분이 미리 주어져 있으니, "이런 상황에는 이렇게 쓰는구나" 정도로 눈에 익혀두면 됩니다. 직접 선택해서 쓰는 연습은 다음 챕터에서 이어집니다.
...
### 요구사항
학생 리스트 \`students\`(각 학생은 \`name\`, \`score\`, \`grade\` 필드를 가짐)에서 점수가 60점 이상인 학생 수를 구하라.
...
\`\`\`java
List<Student> students = Arrays.asList(
    new Student("jimin", 72, 2),
    new Student("hana", 45, 1),
    new Student("suho", 88, 3),
    new Student("yerin", 55, 2)
);
\`\`\`

### 정답
- BLANK1: \`.filter(s -> s.getScore() >= 60)\`

### 해설
"~의 개수를 구하라"는 요구사항은 리스트가 아니라 숫자 하나가 필요하므로, \`collect\`가 아니라 개수를 세는 최종연산인 \`count()\`가 필요합니다. \`count()\`는 \`long\` 타입을 반환합니다. 지금은 \`count()\`가 이미 주어져 있어 사용 형태만 확인하면 되고, 다음 챕터에서는 이런 상황에 직접 \`count()\`를 골라 쓰는 연습을 합니다.

---

id: ch3-07
title: 유효 주문의 총액 합산하기
difficulty: hard
---

### 요구사항
주문 리스트 \`orders\`(각 주문은 \`itemName\`, \`quantity\`, \`price\` 필드를 가짐)에서 수량이 1개 이상이고 가격이 5000원을 넘는 주문만 골라, 각 주문의 총액(수량 × 가격)을 모두 더한 값을 구하라.
...
List<Order> orders = Arrays.asList(
    new Order("notebook", 2, 1500),
    new Order("pen", 10, 800),
    new Order("bag", 1, 32000),
    new Order("eraser", 0, 500)
);
\`\`\`

### 정답
- BLANK1: \`.filter(o -> o.getQuantity() >= 1 && o.getPrice() > 5000)\`
- BLANK2: \`.map(o -> o.getQuantity() * o.getPrice())\`

### 해설
이 문제는 세 단계로 나뉩니다. 먼저 조건("수량 1개 이상이고 가격 5000원 초과")으로 걸러내고(filter), 각 주문을 총액 값 하나로 변환합니다(map). 마지막으로 그 값들을 모두 더하는 부분은 \`reduce(0, (a, b) -> a + b)\`로 미리 주어져 있습니다. \`reduce\`의 첫 번째 인자는 누적을 시작할 초깃값, 두 번째 인자는 두 값을 하나로 합치는 방법을 정의하는 \`BinaryOperator\`입니다. 지금은 형태를 눈에 익히고, 직접 초깃값과 합치는 방법까지 구성하는 연습은 다음 챕터에서 이어집니다.

> 참고: BLANK1처럼 \`&&\`로 두 조건을 연결하는 경우, \`o.getQuantity() >= 1 && o.getPrice() > 5000\`처럼 괄호 없이 작성한 답과 문자열이 정확히 일치해야 정답으로 인정됩니다. \`(o.getQuantity() >= 1) && (o.getPrice() > 5000)\`처럼 조건마다 괄호를 추가로 쳐도 의미는 같지만, 채점기는 괄호 유무를 구분하니 참고해 주세요.
`,te=`# 사전 준비

이 챕터는 3단계에서 만들어 둔 \`Product\`, \`Student\`, \`Order\`, \`Member\` 클래스를 그대로 재사용합니다. 아직 만들지 않았다면 3단계의 "사전 준비" 섹션을 참고해 먼저 만들어 두세요.

---

id: ch4-01
title: 재고 있는 상품 이름 리스트 만들기
difficulty: medium
---

### 요구사항
상품 리스트 \`products\`(각 상품은 \`name\`, \`price\`, \`stock\` 필드를 가짐)에서 재고(\`stock\`)가 0보다 큰 상품의 이름만 모아 리스트로 만들어라.
...
List<Product> products = Arrays.asList(
    new Product("keyboard", 45000, 12),
    new Product("mouse", 8000, 0),
    new Product("monitor", 210000, 3),
    new Product("mousepad", 5000, 0)
);
\`\`\`

### 정답
- BLANK1: \`.filter(p -> p.getStock() > 0)\`
- BLANK2: \`.map(p -> p.getName())\`
- BLANK3: \`.collect(Collectors.toList())\`

### 해설
"~를 모아 리스트로 만들어라"는 요구사항은 결과를 리스트 형태로 모아야 하므로 최종연산으로 \`collect(Collectors.toList())\`가 필요합니다. 지금까지 계속 봐온 형태이지만, 이번엔 최종연산까지 스스로 판단해서 채워야 합니다.

---

id: ch4-02
title: 재고 부족 상품 이름 출력하기
difficulty: medium
---

### 요구사항
상품 리스트 \`products\`(각 상품은 \`name\`, \`price\`, \`stock\` 필드를 가짐)에서 재고가 5개 미만인 상품의 이름을 콘솔에 출력하라. (리스트로 만들지 않고 바로 출력한다)
...
### 정답
- BLANK1: \`.filter(p -> p.getStock() < 5)\`
- BLANK2: \`.forEach(p -> System.out.println(p.getName()))\`

### 해설
값을 모으거나 계산할 필요 없이 각 요소에 대해 동작(출력)만 실행하면 되는 상황이므로 \`forEach\`가 최종연산으로 적합합니다. 3단계에서 형태를 봤던 것을 이번엔 처음부터 끝까지 직접 구성합니다.
...
### 요구사항
회원 리스트 \`members\`(각 회원은 \`id\`, \`nickname\`, \`points\` 필드를 가짐)에서 포인트가 1000점 이상인 회원 수를 구하라.

### 코드
\`\`\`java
long result = members.stream()
    {{BLANK1}}
    {{BLANK2}};
\`\`\`
...
List<Member> members = Arrays.asList(
    new Member("u01", "jimin", 1200),
    new Member("u02", "hana", 850),
    new Member("u03", "suho", 3000),
    new Member("u04", "yerin", 400)
);
\`\`\`

### 정답
- BLANK1: \`.filter(m -> m.getPoints() >= 1000)\`
- BLANK2: \`.count()\`

### 해설
"~의 수를 구하라"는 요구사항은 개수(숫자 하나)가 필요하므로 \`count()\`가 최종연산으로 적합합니다. \`count()\`는 \`long\` 타입을 반환한다는 점을 기억해두세요.

---

id: ch4-04
title: 장바구니 총액 구하기 (reduce - 누적 방법만 직접)
difficulty: medium
---

### 요구사항
장바구니에 담긴 상품 리스트 \`cartItems\`(각 상품은 \`name\`, \`price\`, \`stock\` 필드를 가짐)에서 모든 상품의 가격을 더한 총액을 구하라.

### 코드
\`\`\`java
int result = cartItems.stream()
...
List<Product> cartItems = Arrays.asList(
    new Product("keyboard", 45000, 1),
    new Product("mouse", 8000, 1),
    new Product("mousepad", 5000, 1)
);
\`\`\`
...
### 해설
여러 값을 하나의 값으로 누적해서 합칠 때는 \`reduce\`를 사용합니다. 첫 번째 인자(\`0\`)는 누적을 시작할 초깃값이고, 두 번째 인자는 이전까지의 누적값(\`a\`)과 새로 만나는 값(\`b\`)을 어떻게 합칠지 정의하는 \`BinaryOperator\`입니다. 덧셈이므로 초깃값은 더해도 값이 변하지 않는 \`0\`이 자연스럽게 쓰이며, 이번 문제에서는 초깃값은 미리 주어져 있고 누적 방법만 직접 구성합니다.

---

id: ch4-05
title: 최고 점수 찾기 (reduce - 초깃값과 누적 방법 모두 직접)
difficulty: hard
---

### 요구사항
정수 리스트 \`scores\`에서 가장 큰 값을 찾아라. (리스트가 비어있지 않다고 가정한다)

### 코드
\`\`\`java
int result = scores.stream()
    .reduce({{BLANK1}}, {{BLANK2}});
\`\`\`

### 실행용 데이터
\`\`\`java
List<Integer> scores = Arrays.asList(72, 88, 95, 61, 84);
\`\`\`

### 정답
- BLANK1: \`Integer.MIN_VALUE\`
- BLANK2: \`(a, b) -> a > b ? a : b\`

### 해설
이번엔 초깃값도 직접 정해야 합니다. "최댓값 찾기"에서 초깃값은 어떤 값과 비교해도 항상 작은 값이어야 하므로 \`Integer.MIN_VALUE\`가 안전한 선택입니다(0으로 하면 모든 값이 음수인 리스트에서 잘못된 결과가 나옵니다). 누적 방법은 두 값 중 더 큰 값을 선택하는 삼항연산자로 표현합니다. 덧셈처럼 초깃값이 항상 \`0\`으로 고정되는 게 아니라, "어떤 연산을 하느냐"에 따라 초깃값도 달라진다는 점이 이 문제의 핵심입니다.

---

id: ch4-06
title: 태그를 하나의 문자열로 합치기
difficulty: medium
---

### 요구사항
문자열 리스트 \`tags\`의 모든 태그를 쉼표와 공백(", ")으로 구분해 이어붙인 하나의 문자열로 만들어라. 단, 길이가 2 이하인 태그는 제외한다.
...
### 실행용 데이터
\`\`\`java
List<String> tags = Arrays.asList("java", "ai", "spring", "db", "backend");
\`\`\`

### 정답
- BLANK1: \`.filter(t -> t.length() > 2)\`

### 해설
여러 문자열을 구분자로 이어붙여 하나의 문자열로 만들 때는 \`Collectors.joining(구분자)\`를 사용합니다. 이 문제에서는 \`joining\` 부분은 미리 주어져 있으니 형태를 눈에 익히고, 그 앞에서 조건에 맞는 태그만 걸러내는 \`filter\`를 직접 구성하면 됩니다.

---

id: ch4-07
title: 학년별로 학생 묶기 (참고 예제)
difficulty: hard
---

### 요구사항
학생 리스트 \`students\`(각 학생은 \`name\`, \`score\`, \`grade\` 필드를 가짐)를 학년(\`grade\`)별로 묶어서 \`Map<Integer, List<Student>>\` 형태로 만들어라.
...
List<Student> students = Arrays.asList(
    new Student("jimin", 95, 2),
    new Student("hana", 82, 3),
    new Student("suho", 91, 2),
    new Student("yerin", 77, 1)
);
\`\`\`

### 정답
이 문제는 빈칸이 없습니다. 코드를 읽고 동작 방식을 이해하는 참고 예제입니다.

### 해설
\`Collectors.groupingBy()\`는 요소를 특정 기준으로 분류해서 \`Map\`으로 묶어주는 Collector입니다. \`s -> s.getGrade()\`는 분류 기준(어떤 값을 키로 쓸지)을 정하는 \`Function\`이며, 그 결과 같은 학년끼리 \`List<Student>\`로 묶여 \`Map<Integer, List<Student>>\`가 만들어집니다. 지금까지 결과 타입이 항상 \`List\`나 단일 값이었다면, \`groupingBy\`는 처음으로 \`Map\`이라는 새로운 결과 형태를 만든다는 점만 기억해두세요. 직접 다뤄보는 연습은 이후 심화 학습에서 이어집니다.

---

id: ch4-08
title: 학년별 학생 수 세기 (참고 예제)
difficulty: hard
---

### 요구사항
학생 리스트 \`students\`(각 학생은 \`name\`, \`score\`, \`grade\` 필드를 가짐)를 학년별로 묶되, 학생 목록이 아니라 각 학년의 인원수를 구해 \`Map<Integer, Long>\` 형태로 만들어라.
...
### 해설
\`groupingBy\`는 두 번째 인자로 또 다른 Collector를 받을 수 있습니다. 이 경우 각 그룹(학년)에 속한 학생들을 리스트로 모으는 대신, \`Collectors.counting()\`을 함께 써서 그룹별 인원수만 계산합니다. Collector 안에 Collector가 중첩되는 구조라 지금까지 본 것 중 가장 복잡해 보일 수 있지만, "그룹핑 기준"과 "그룹 내에서 할 집계 방법"이 각각 첫 번째, 두 번째 인자로 분리되어 있다는 흐름만 이해하면 충분합니다. 지금은 이런 조합이 가능하다는 것만 알아두고 넘어가도 좋습니다.

---

id: ch4-09
title: 조건에 맞는 첫 학생 찾기 (findFirst)
difficulty: medium
---

### 요구사항
학생 리스트 \`students\`(각 학생은 \`name\`, \`score\`, \`grade\` 필드를 가짐)에서 점수가 90점 이상인 학생 중 첫 번째 학생을 찾아라.
...
### 해설
지금까지 본 최종연산(\`collect\`, \`forEach\`, \`count\`, \`reduce\`)은 모두 확실히 값이 있는 결과를 반환했지만, \`findFirst()\`는 조건에 맞는 요소가 하나도 없을 수도 있습니다. 그래서 결과 타입이 \`Student\`가 아니라 **\`Optional<Student>\`**입니다. \`Optional\`은 "값이 있을 수도, 없을 수도 있다"는 상태를 표현하는 컨테이너입니다. 이 값을 안전하게 꺼내 쓰는 두 가지 대표적인 방법을 아래 참고 코드로 확인해보세요.

### 참고 코드
\`\`\`java
// 방법 1 - 값이 있을 때만 실행 (반환값 없음)
Optional<Student> result = students.stream()
    .filter(s -> s.getScore() >= 90)
    .findFirst();
result.ifPresent(s -> System.out.println(s.getName()));

// 방법 2 - 값이 없으면 기본값 사용 (반환값 있음)
Student defaultStudent = new Student("없음", 0, 0);
Student topStudent = result.orElse(defaultStudent);

// 실무에서는 아래처럼 변수에 담지 않고 바로 이어서 쓰는 경우가 더 흔합니다
students.stream()
    .filter(s -> s.getScore() >= 90)
    .findFirst()
    .ifPresent(s -> System.out.println(s.getName()));
\`\`\`
값이 있을 때만 어떤 동작(출력, 저장 등)을 실행하고 끝내려면 \`ifPresent\`를, 값을 꺼내서 다른 곳에 계속 활용하려면 \`orElse\`를 사용합니다. 바로 다음 두 문제에서 각각을 직접 연습해봅니다.

---

id: ch4-10
title: 조건에 맞는 학생이 있으면 출력하기 (ifPresent)
difficulty: medium
---

### 요구사항
학생 리스트 \`students\`(각 학생은 \`name\`, \`score\`, \`grade\` 필드를 가짐)에서 점수가 90점 이상인 학생 중 첫 번째 학생이 있으면 그 이름을 콘솔에 출력하라.
...
- BLANK1: \`.filter(s -> s.getScore() >= 90)\`
- BLANK2: \`ifPresent\`
- BLANK3: \`s -> System.out.println(s.getName())\`

### 해설
\`ifPresent\`는 \`Optional\` 안에 값이 있을 때만 주어진 동작을 실행하고, 값이 없으면 아무 일도 하지 않습니다. 매개변수로 \`Consumer\`를 받는다는 점에서 1단계에서 배운 \`Consumer\` 문법을 그대로 재사용하는 것뿐입니다. \`ifPresent\`는 반환값이 없으므로, "실행하고 끝나는" 상황에 적합합니다.

---

id: ch4-11
title: 닉네임을 찾아 기본값과 함께 반환하기 (orElse)
difficulty: medium
---

### 요구사항
회원 리스트 \`members\`(각 회원은 \`id\`, \`nickname\`, \`points\` 필드를 가짐)에서 \`id\`가 \`targetId\`와 일치하는 회원의 닉네임을 찾아라. 만약 해당하는 회원이 없다면 "닉네임 없음"을 대신 사용하라.
...
### 정답
- BLANK1: \`.filter(m -> m.getId().equals(targetId))\`
- BLANK2: \`.map(m -> m.getNickname())\`
- BLANK3: \`orElse\`
- BLANK4: \`"닉네임 없음"\`
...
`;function ne(e){let t=null,n=e,r=e.match(/^# 사전 준비\n([\s\S]*?)\n---\n\n/);r?(t=r[1].trim(),n=e.slice(r[0].length)):n=n.replace(/^---\n/,``);let i=n.split(/\n---\n\n(?=id: )/).map(re).filter(Boolean);return{intro:t,problems:i}}function re(e){let t=e.match(/^id:\s*(.+)\ntitle:\s*(.+)\ndifficulty:\s*(.+)\n---\n\n([\s\S]*)$/);if(!t)return null;let[,n,r,i,a]=t,o=w(a),s=o.코드||``,c=oe(s),l=c.length===0,u=se(o.정답||``);return{id:n.trim(),title:r.trim(),difficulty:i.trim(),requirement:(o.요구사항||``).trim(),code:ie(s),execData:o[`실행용 데이터`]?ie(o[`실행용 데이터`]):null,answers:u,explanation:(o.해설||``).trim(),refCode:o[`참고 코드`]?ie(o[`참고 코드`]):null,refCodeNote:o[`참고 코드`]?ae(o[`참고 코드`]):``,blanks:c,isReferenceType:l}}function w(e){let t=[`요구사항`,`코드`,`실행용 데이터`,`정답`,`해설`,`참고 코드`],n={},r=RegExp(`### (${t.join(`|`)})\\n`,`g`),i=[...e.matchAll(r)];for(let t=0;t<i.length;t++){let r=i[t][1],a=i[t].index+i[t][0].length,o=t+1<i.length?i[t+1].index:e.length;n[r]=e.slice(a,o).trim()}return n}function ie(e){let t=e.match(/```java\n([\s\S]*?)```/);return t?t[1].replace(/\n$/,``):e}function ae(e){let t=e.indexOf("```",e.indexOf("```")+3);return t>=0?e.slice(t+3).trim():``}function oe(e){let t=[...e.matchAll(/\{\{(BLANK\d+)\}\}/g)],n=new Set,r=[];for(let e of t)n.has(e[1])||(n.add(e[1]),r.push(e[1]));return r}function se(e){let t={},n=e.split(`
`);for(let e of n){let n=e.match(/^-\s*(BLANK\d+):\s*`(.+)`\s*$/);n&&(t[n[1]]=n[2])}return t}function ce(e){if(e==null)return``;let t=String(e);return t=t.replace(/\s+/g,``),t=t.replace(/(^|[(,])\(([a-zA-Z_$][a-zA-Z0-9_$]*)\)->/g,`$1$2->`),t=le(t),t}function le(e){let t=[...e.matchAll(/(?<![\w.$])(\([a-zA-Z_$][\w$]*(?:,[a-zA-Z_$][\w$]*)*\)|\(\)|[a-zA-Z_$][\w$]*)->/g)];if(t.length===0)return e;let n=``,r=0;for(let i=0;i<t.length;i++){let a=t[i],o=a[1],s=a.index+a[0].length,c=T(e,s,i+1<t.length?t[i+1].index:e.length),l=e.slice(r,a.index),u=e.slice(s,c);u=ue(u);let{newParamPart:d,newBody:f}=E(o,u);n+=l+d+`->`+f,r=c}return n+=e.slice(r),n}function ue(e){if(!e.startsWith(`{`)||!e.endsWith(`}`))return e;let t=e.slice(1,-1),n=t.match(/^return(.+);$/);return n?n[1]:t.endsWith(`;`)&&!t.slice(0,-1).includes(`;`)?t.slice(0,-1):e}function T(e,t,n){if(e[t]===`{`){let r=0;for(let i=t;i<n;i++)if(e[i]===`{`)r++;else if(e[i]===`}`&&(r--,r===0))return i+1;return n}let r=0;for(let i=t;i<n;i++){let t=e[i];if(t===`(`||t===`[`)r++;else if(t===`)`||t===`]`){if(r===0)return i;r--}else if(t===`,`&&r===0)return i}return n}function E(e,t){let n;if(e.startsWith(`(`)){let t=e.slice(1,-1);n=t===``?[]:t.split(`,`).map(e=>e.trim())}else n=[e];if(n.length===0)return{newParamPart:`()`,newBody:t};let r=t;return n.forEach((e,t)=>{let n=`$${t+1}`,i=RegExp(`(?<![a-zA-Z0-9_$])${de(e)}(?![a-zA-Z0-9_$])`,`g`);r=r.replace(i,n)}),{newParamPart:n.length===1?`$1`:`(${n.map((e,t)=>`$${t+1}`).join(`,`)})`,newBody:r}}function de(e){return e.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`)}function fe(e,t){return ce(e)===ce(t)}function pe(e,t){if(!e.execData)return null;let n=e.code;for(let r of e.blanks){let e=t[r];if(!e||!e.trim())return null;n=n.split(`{{${r}}}`).join(e)}let r=[e.execData,n],i=n.trim().split(`
`)[0].match(/^[\w<>,\s\[\]]+\s+(\w+)\s*=/);return i&&r.push(`System.out.println(${i[1]});`),r.join(`

`)}var me={easy:`쉬움`,medium:`보통`,hard:`어려움`};function D({level:e}){return(0,x.jsx)(`span`,{className:`diff-badge diff-${e}`,children:me[e]||e})}function O({text:e,disabled:t,label:n}){let[r,i]=(0,b.useState)(!1);return(0,x.jsx)(`button`,{className:`copy-btn`,onClick:()=>{t||!e||(navigator.clipboard&&navigator.clipboard.writeText&&navigator.clipboard.writeText(e).catch(()=>{}),i(!0),setTimeout(()=>i(!1),1500))},disabled:t,title:t?`빈칸을 모두 채우면 복사할 수 있어요`:`실행 가능한 코드로 복사`,children:r?`복사됨 ✓`:n||`복사`})}function he({children:e}){return(0,x.jsx)(`pre`,{className:`code-block`,children:(0,x.jsx)(`code`,{children:e})})}function ge({code:e,values:t,results:n,onChange:r}){let i=[],a=0,o=/\{\{(BLANK\d+)\}\}/g,s,c=0;for(;(s=o.exec(e))!==null;){i.push((0,x.jsx)(`span`,{children:e.slice(a,s.index)},c++));let o=s[1],l=n?n[o]:void 0,u=t[o]||``;i.push((0,x.jsx)(`input`,{className:`blank-input`+(l===!0?` blank-correct`:l===!1?` blank-wrong`:``),value:u,onChange:e=>r(o,e.target.value),spellCheck:!1,style:{width:Math.max(u.length,6)+2+`ch`},placeholder:`입력`},c++)),a=s.index+s[0].length}return i.push((0,x.jsx)(`span`,{children:e.slice(a)},c++)),(0,x.jsx)(`pre`,{className:`code-block`,children:(0,x.jsx)(`code`,{children:i})})}function _e({problem:e,chapterTitle:t,onSolved:n,isSolved:r,onPrev:i,onNext:a,hasPrev:o,hasNext:s,currentIndex:c,totalCount:l}){let[u,d]=(0,b.useState)({}),[f,p]=(0,b.useState)(null),[m,h]=(0,b.useState)(!1),[g,_]=(0,b.useState)(!1);(0,b.useMemo)(()=>{d({}),p(null),h(!1),_(!1)},[e.id]);let v=(e,t)=>{d(n=>({...n,[e]:t})),p(null)},y=()=>{let t={},r=!0;for(let n of e.blanks){let i=fe(u[n]||``,e.answers[n]||``);t[n]=i,i||(r=!1)}p(t),r&&n(e.id)},ee=e.blanks.every(e=>(u[e]||``).trim().length>0),S=(0,b.useMemo)(()=>pe(e,u),[e,u]);if(e.isReferenceType)return(0,x.jsxs)(`div`,{className:`problem-view`,children:[(0,x.jsxs)(`div`,{className:`problem-header`,children:[(0,x.jsxs)(`div`,{className:`problem-header-top`,children:[(0,x.jsx)(`span`,{className:`chapter-crumb`,children:t}),(0,x.jsxs)(`span`,{className:`position-badge`,children:[c+1,` / `,l]}),(0,x.jsx)(D,{level:e.difficulty}),(0,x.jsx)(`span`,{className:`ref-tag`,children:`참고 예제`})]}),(0,x.jsx)(`h2`,{children:e.title})]}),(0,x.jsx)(ye,{label:`요구사항`,children:(0,x.jsx)(`p`,{children:e.requirement})}),(0,x.jsx)(ye,{label:`코드`,action:(0,x.jsx)(O,{text:e.code,label:`복사`}),children:(0,x.jsx)(he,{children:e.code})}),(0,x.jsx)(ye,{label:`해설`,children:(0,x.jsx)(`p`,{children:e.explanation})}),(0,x.jsx)(ve,{onPrev:i,onNext:a,hasPrev:o,hasNext:s})]});let C=f&&Object.values(f).every(Boolean);return(0,x.jsxs)(`div`,{className:`problem-view`,children:[(0,x.jsxs)(`div`,{className:`problem-header`,children:[(0,x.jsxs)(`div`,{className:`problem-header-top`,children:[(0,x.jsx)(`span`,{className:`chapter-crumb`,children:t}),(0,x.jsxs)(`span`,{className:`position-badge`,children:[c+1,` / `,l]}),(0,x.jsx)(D,{level:e.difficulty}),r&&(0,x.jsx)(`span`,{className:`solved-tag`,children:`✓ 완료`})]}),(0,x.jsx)(`h2`,{children:e.title})]}),(0,x.jsx)(ye,{label:`요구사항`,children:(0,x.jsx)(`p`,{children:e.requirement})}),(0,x.jsx)(ye,{label:`코드`,action:e.execData?(0,x.jsx)(O,{text:S,disabled:!ee||!S}):null,children:(0,x.jsx)(ge,{code:e.code,values:u,results:f,onChange:v})}),(0,x.jsxs)(`div`,{className:`action-row`,children:[(0,x.jsx)(`button`,{className:`check-btn`,onClick:y,disabled:!ee,children:`정답 확인`}),f&&(0,x.jsx)(`span`,{className:C?`result-ok`:`result-fail`,children:C?`정답입니다! 🎉`:`다시 확인해보세요`})]}),f&&(0,x.jsxs)(x.Fragment,{children:[(0,x.jsx)(ye,{label:`해설`,action:(0,x.jsx)(`button`,{className:`toggle-btn`,onClick:()=>h(e=>!e),children:m?`접기`:`해설 보기`}),children:m&&(0,x.jsxs)(x.Fragment,{children:[(0,x.jsxs)(`div`,{className:`answer-box`,children:[(0,x.jsx)(`div`,{className:`answer-box-title`,children:`정답`}),e.blanks.map(t=>(0,x.jsxs)(`div`,{className:`answer-line`,children:[e.blanks.length>1&&(0,x.jsx)(`span`,{className:`answer-blank-id`,children:t}),(0,x.jsx)(`code`,{children:e.answers[t]})]},t))]}),(0,x.jsx)(`p`,{children:e.explanation})]})}),e.refCode&&(0,x.jsx)(ye,{label:`참고 코드`,action:(0,x.jsx)(`button`,{className:`toggle-btn`,onClick:()=>_(e=>!e),children:g?`접기`:`펼치기`}),children:g&&(0,x.jsxs)(x.Fragment,{children:[(0,x.jsx)(he,{children:e.refCode}),e.refCodeNote&&(0,x.jsx)(`p`,{className:`ref-note`,children:e.refCodeNote})]})})]}),(0,x.jsx)(ve,{onPrev:i,onNext:a,hasPrev:o,hasNext:s})]})}function ve({onPrev:e,onNext:t,hasPrev:n,hasNext:r}){return(0,x.jsxs)(`div`,{className:`nav-row`,children:[(0,x.jsx)(`button`,{onClick:e,disabled:!n,children:`← 이전 문제`}),(0,x.jsx)(`button`,{onClick:t,disabled:!r,children:`다음 문제 →`})]})}function ye({label:e,action:t,children:n}){return(0,x.jsxs)(`div`,{className:`section`,children:[(0,x.jsxs)(`div`,{className:`section-head`,children:[(0,x.jsx)(`span`,{className:`section-label`,children:e}),t]}),n]})}function be({intro:e}){if(!e)return null;let t=e.split("```java\n"),n=t[0],r=t.slice(1);return(0,x.jsxs)(`div`,{className:`intro-box`,children:[(0,x.jsx)(`div`,{className:`intro-title`,children:`📦 사전 준비`}),n.trim()&&(0,x.jsx)(`p`,{children:n.trim()}),r.map((e,t)=>{let n=e.indexOf("```"),r=e.slice(0,n).replace(/\n$/,``),i=e.slice(n+3);return(0,x.jsxs)(b.Fragment,{children:[(0,x.jsxs)(`div`,{className:`intro-code-wrap`,children:[(0,x.jsx)(he,{children:r}),(0,x.jsx)(O,{text:r,label:`복사`})]}),i.trim()&&!i.trim().startsWith("```java")&&(0,x.jsx)(`p`,{children:i.trim()})]},t)})]})}function xe({chapters:e,onSelect:t}){return(0,x.jsxs)(`div`,{className:`chapter-list`,children:[(0,x.jsxs)(`div`,{className:`hero`,children:[(0,x.jsx)(`span`,{className:`hero-eyebrow`,children:`JAVA PRACTICE`}),(0,x.jsx)(`h1`,{children:`람다 · 스트림 · Optional`}),(0,x.jsx)(`p`,{className:`subtitle`,children:`빈칸을 채우며 손으로 익히는 함수형 문법 연습`})]}),(0,x.jsx)(`div`,{className:`chapter-cards`,children:e.map((e,n)=>(0,x.jsxs)(`button`,{className:`chapter-card`,onClick:()=>t(n),children:[(0,x.jsx)(`span`,{className:`chapter-num`,children:String(n+1).padStart(2,`0`)}),(0,x.jsxs)(`div`,{className:`chapter-card-body`,children:[(0,x.jsx)(`span`,{className:`chapter-title`,children:e.title}),(0,x.jsxs)(`span`,{className:`chapter-count`,children:[e.problems.length,`문제`]})]}),(0,x.jsx)(`span`,{className:`chapter-arrow`,children:`→`})]},e.key))})]})}function Se({chapter:e,solvedIds:t,onSelectProblem:n,onBack:r}){let i=e.problems.filter(e=>!e.isReferenceType&&t.has(e.id)).length,a=e.problems.filter(e=>!e.isReferenceType).length;return(0,x.jsxs)(`div`,{className:`problem-list`,children:[(0,x.jsx)(`button`,{className:`back-btn`,onClick:r,children:`← 챕터 목록`}),(0,x.jsx)(`h2`,{children:e.title}),(0,x.jsx)(`div`,{className:`progress-line`,children:a>0?`${i} / ${a} 문제 완료`:``}),(0,x.jsx)(be,{intro:e.intro}),(0,x.jsx)(`ul`,{className:`problem-items`,children:e.problems.map((e,r)=>(0,x.jsx)(`li`,{children:(0,x.jsxs)(`button`,{className:`problem-item`,onClick:()=>n(r),children:[(0,x.jsx)(`span`,{className:`problem-item-num`,children:r+1}),(0,x.jsx)(`span`,{className:`problem-item-title`,children:e.title}),(0,x.jsx)(D,{level:e.difficulty}),e.isReferenceType?(0,x.jsx)(`span`,{className:`ref-tag small`,children:`참고`}):t.has(e.id)?(0,x.jsx)(`span`,{className:`solved-dot`,children:`✓`}):(0,x.jsx)(`span`,{className:`unsolved-dot`})]})},e.id))})]})}function Ce(){let e=[{key:`ch1`,title:`1단계 · 함수형 인터페이스`,raw:ee},{key:`ch2`,title:`2단계 · map / filter 빈칸 채우기`,raw:S},{key:`ch3`,title:`3단계 · 연산 선택과 구성`,raw:C},{key:`ch4`,title:`4단계 · 최종연산 직접 선택`,raw:te}],t=(0,b.useMemo)(()=>e.map(({key:e,title:t,raw:n})=>{let{intro:r,problems:i}=ne(n);return{key:e,title:t,intro:r,problems:i}}),[]),[n,r]=(0,b.useState)({screen:`chapters`}),[i,a]=(0,b.useState)(new Set),o=e=>{a(t=>new Set(t).add(e))},s;if(n.screen===`chapters`)s=(0,x.jsx)(xe,{chapters:t,onSelect:e=>r({screen:`list`,chapterIdx:e})});else{let e=t[n.chapterIdx];if(n.screen===`list`)s=(0,x.jsx)(Se,{chapter:e,solvedIds:i,onSelectProblem:e=>r({screen:`problem`,chapterIdx:n.chapterIdx,problemIdx:e}),onBack:()=>r({screen:`chapters`})});else{let t=e.problems[n.problemIdx],a=n.problemIdx>0,c=n.problemIdx<e.problems.length-1;s=(0,x.jsxs)(x.Fragment,{children:[(0,x.jsxs)(`button`,{className:`back-btn`,onClick:()=>r({screen:`list`,chapterIdx:n.chapterIdx}),children:[`← `,e.title,` 목록`]}),(0,x.jsx)(_e,{problem:t,chapterTitle:e.title,onSolved:o,isSolved:i.has(t.id),hasPrev:a,hasNext:c,currentIndex:n.problemIdx,totalCount:e.problems.length,onPrev:()=>r({...n,problemIdx:n.problemIdx-1}),onNext:()=>r({...n,problemIdx:n.problemIdx+1})})]})}}return(0,x.jsxs)(`div`,{className:`app-root`,children:[(0,x.jsx)(`style`,{children:we}),(0,x.jsx)(`div`,{className:`app`,children:s})]})}var we=`
:root {
  --bg: #16161d;
  --bg-panel: #1e1e28;
  --bg-panel-2: #23232f;
  ...
  --ok: #4ade80;
  --fail: #fb7185;
  --code-font: 'JetBrains Mono', 'Fira Code', ui-monospace, SFMono-Regular, Menlo, monospace;
  --body-font: -apple-system, BlinkMacSystemFont, 'Segoe UI', Pretendard, sans-serif;
}
...
.ref-tag.small { font-size: 10px; padding: 1px 6px; }
...
.diff-easy { color: var(--diff-easy); border-color: var(--diff-easy); }
.diff-medium { color: var(--diff-medium); border-color: var(--diff-medium); }
.diff-hard { color: var(--diff-hard); border-color: var(--diff-hard); }
...
.blank-input:focus { border-style: solid; }
.blank-correct {
...
.copy-btn:hover:not(:disabled) { color: var(--text); border-color: var(--text-dim); }
...
.check-btn:hover:not(:disabled) { opacity: 0.88; }
.check-btn:disabled { opacity: 0.3; cursor: not-allowed; }

.result-ok { color: var(--ok); font-weight: 700; font-size: 14px; }
.result-fail { color: var(--fail); font-weight: 700; font-size: 14px; }
...
`;(0,y.createRoot)(document.getElementById(`root`)).render((0,x.jsx)(b.StrictMode,{children:(0,x.jsx)(Ce,{})}));